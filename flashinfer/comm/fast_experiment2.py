...
def _symm_ptrs(symm_tensor: torch.Tensor, group_name: str, chunk_elems: int | None = None, offset_elems: int = 0):
    """
    Rendezvous once and build the peer pointer table expected by the CUDA kernel.
    If chunk_elems is provided, each peer’s slice is offset accordingly.
    """
    handle = symm_mem.rendezvous(symm_tensor, group=group_name)
    tp_size = handle.world_size
    ptrs = []
    for peer in range(tp_size):
        storage_offset = offset_elems if chunk_elems is None else offset_elems + peer * chunk_elems
        buf = handle.get_buffer(peer, (symm_tensor.numel(),), symm_tensor.dtype, storage_offset=storage_offset)
...
def _run_correctness_worker(world_size, rank, dtype, distributed_init_port):
    ...
    try:
        ...
        # buffer_bytes = world_size * maxTokenNum * maxHiddenSize * 4
        # flag_bytes = (MAX_ALL_REDUCE_BLOCKS + 1) * 4 * world_size * 2
        # lamport_bytes = world_size * LamportTokenNumThreshold * world_size * maxHiddenSize * 2
        # elem_size = torch.float32().itemsize
        # buffer_elems = buffer_size // elem_size
        # buffers = [symm_mem.empty(buffer_elems, dtype=torch.float32, device=f"cuda:{rank}") for _ in range(2)]

        # flag_elems = flag_size // torch.int32().itemsize
        # flags = [symm_mem.empty(flag_elems, dtype=torch.int32, device=f"cuda:{rank}") for _ in range(2)]

        # lamport_elem_size = torch.float16().itemsize
        # lamport_elems = lamport_buffer_size // lamport_elem_size
        # lamports = [symm_mem.empty(lamport_elems, dtype=torch.float16, device=f"cuda:{rank}") for _ in range(3)]
        # end of symm mem workspace
                        ...
                        for fusion_op_code in fusion_op_codes:
                            for launch_with_pdl in launch_with_pdls:
                                ...
                                for _ in range(test_loop):
                                    ...
                                    # lamport_chunk = LamportTokenNumThreshold * world_size * hidden_size
                                    # comm.trtllm_custom_all_reduce(
                                    #     inp=inp1,
                                    #     out=out1,
                                    #     tp_size=world_size,
                                    ...
                                    #     launch_with_pdl=launch_with_pdl,
                                    #     flag_value=flag_value,
                                    #     peer_comm_buffer_ptrs=_symm_ptrs(buffers[0], group_name),
                                    #     peer_barrier_ptrs_in=_symm_ptrs(flags[0], group_name),
                                    #     peer_barrier_ptrs_out=_symm_ptrs(flags[1], group_name),
                                    #     bias=bias,
                                    #     residual=residual,
                                    #     weight=weight,
                                    #     weight_pre_residual_norm=weight_pre_residual_norm,
                                    #     eps=eps,
                                    #     intermediate_buffer=intermediate_buffer,
                                    #     lamport_peer_comm_buffer_ptrs_0=_symm_ptrs(lamports[0], group_name, chunk_elems=lamport_chunk),
                                    #     lamport_peer_comm_buffer_ptrs_1=_symm_ptrs(lamports[1], group_name, chunk_elems=lamport_chunk),
                                    #     lamport_peer_comm_buffer_ptrs_2=_symm_ptrs(lamports[2], group_name, chunk_elems=lamport_chunk),
                                    # )
